...
the Lilypond equivalent in a string."""
	...
	# Handle the Bars:
	result = ""
	clef = track.instrument.clef
	result += "\clef " + clef

	if track.chords != []:
		result += ' <<\chords {'
		
		def transcribe_chords(cs, depth = 0):
			final = ''
			for c in cs:
				if type(c) == list:
					f = transcribe_chords(c, depth + 1)
				if type(c) == str:
					length = (2**depth)
					if len(c) != 1 and c[1] in ['b','#']:
						root = c[:1].lower() + {'b':'es','#':'is'}[c[1]]
						rest = c[2:].replace('o','dim').replace('0','dim').replace('O','dim')
					else:
						root = c[0].lower()
						rest = c[1:].replace('o','dim').replace('0','dim').replace('O','dim')
					if rest:
						f = root + str(length) + ':' + rest + ' '
					else:
						f = root+ str(length) + ' '
				final += f
			return final
		result += transcribe_chords(track.chords)
		result += '}{'
...
[refMingusContainersComposition Composition] in a string"""
	...
	result = "\\header { title = \"%s\" piece=\"Bass\" subtitle =\"(Generated by PyWalkingBass)\" composer = \"%s\" opus = \"%s\" } "\
